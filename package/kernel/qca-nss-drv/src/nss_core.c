...
(((LINUX_VERSION_CODE >= KERNEL_VERSION(4, 4, 0)) && (LINUX_VERSION_CODE < KERNEL_VERSION(4, 15, 0))))))
 ...
 */
static refcount_t jumbo_mru;
static refcount_t paged_mode;
...
{
	return refcount_read(&jumbo_mru);
}
...
{
	return refcount_read(&paged_mode);
}
