...
struct nss_capwap_handle {
	refcount_t refcnt;			/**< Reference count on the tunnel */
...
static void nss_capwap_refcnt_inc(int32_t if_num)
{
	...
	refcount_inc(&nss_capwap_hdl[if_num]->refcnt);
	nss_assert(refcount_read(&nss_capwap_hdl[if_num]->refcnt) > 0);
}
...
static void nss_capwap_refcnt_dec(int32_t if_num)
{
	if_num = if_num - NSS_DYNAMIC_IF_START;
	nss_assert(refcount_read(&nss_capwap_hdl[if_num]->refcnt) > 0);
	refcount_dec(&nss_capwap_hdl[if_num]->refcnt);
}
...
{
	if_num = if_num - NSS_DYNAMIC_IF_START;
	return refcount_read(&nss_capwap_hdl[if_num]->refcnt);
}
